...
foreach ($inputArray as $inputElement) {
  $spacePos = strpos($inputElement, " ");
  $openParenthesisPos = strpos($inputElement, "(");
  $closeParenthesisPos = strpos($inputElement, ")");
  $programsName = substr($inputElement, 0, $spacePos);
  $hyphenPos = strpos($inputElement, "-");

  $weight = (int) substr($inputElement, $openParenthesisPos + 1, $closeParenthesisPos - $openParenthesisPos -1);
  $tree[$programsName]["dependencies"] = [];
  $tree[$programsName]['weight'] = $weight;

  // Si l'élément a des dépendances
  if ($hyphenPos !== false) {
    $tree[$programsName]['dependencies'] = explode(", ", substr($inputElement, $hyphenPos + 3, strlen($input)-1));
  }
}
...
foreach ($tree as $value) {
  $dependencies = array_merge($dependencies, $value['dependencies']);
}
